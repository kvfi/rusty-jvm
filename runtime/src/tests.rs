...
fn check_simple_fields_methods() {
    let bytes = match file_to_bytes(Path::new(
        "../resources/test/rjvmtest/SimpleFieldsAndMethods.class",
    )) {
        Ok(buf) => buf,
        _ => panic!("Error reading SimpleFieldsAndMethods"),
    };
    let mut parser =
        klass_parser::OtKlassParser::of(bytes, "rjvmtest/SimpleFieldsAndMethods.class".to_string());
    parser.parse();
    assert_eq!(23, parser.get_pool_size());
    let k = parser.klass();
    assert_eq!("rjvmtest/SimpleFieldsAndMethods", k.get_name());
    assert_eq!("java/lang/Object", k.get_super_name());
    assert_eq!(2, k.get_methods().len());
}
